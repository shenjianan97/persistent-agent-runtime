...
#### A2. Impacted Components / Modules
  ...
  Description: Implement the exact Phase 1 PostgreSQL schema and key query support described in `design/PHASE1_DURABLE_EXECUTION.md`, including `tasks`, `checkpoints`, and `checkpoint_writes`, queue/reaper/dead-letter indexes, `updated_at` maintenance, and `LISTEN/NOTIFY` support for claimable task transitions.

  Component: API Service
  Change type: new code
  Path: `src/api-service/src/main/java/` and `src/api-service/src/main/resources/`
  ...
  Change type: new code
  Path: `src/worker-service/checkpointer/`
  ...
  Change type: new code
  Path: `src/worker-service/executor/`
...
When assigning tasks to implementation agents:
  ...
  Use `design/PHASE1_DURABLE_EXECUTION.md` as the canonical architecture/behavior contract and `agent_tasks/*.md` as the task-local implementation contract.
