...
- Treat `design/PHASE1_DURABLE_EXECUTION.md` as the canonical checkpointer behavior contract. The important safety rule is lease-aware checkpoint writes, not speculative library redesign.
...
- **File paths (if known):** `src/worker-service/checkpointer/`
