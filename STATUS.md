...
| 2 | Planning Primitive | **✅ Done & merged (#115, 2026-06-08); archived** — P1–P5 shipped, two-stage reviewed, browser gate passed. `plan_write` ships as a **base platform tool** for all agents (product decision superseded the opt-in design). Outstanding follow-ups (non-blocking, in the track's progress.md): prompt-cache scan dedup; API-wide 405 hygiene; missing-`TAVILY_API_KEY` observability | [plan](docs/exec-plans/completed/agent-modes/planning-primitive/plan.md) | [progress](docs/exec-plans/completed/agent-modes/planning-primitive/progress.md) |
