...
- **Direct commands vs. make — the split is infra, not preference.** For a narrow **no-infra** scope, invoke the test runner directly: `services/worker-service/.venv/bin/python -m pytest services/worker-service/tests/<file>.py` (pinned venv, never bare `python3`) or `cd services/api-service && ./gradlew test --tests '<Pattern>'`. The moment a test needs **any infra** (DB, API server, S3/LocalStack, embedding mock) the `make` target is **mandatory** — `make worker-test` / `make e2e-test PYTEST_ARGS='-k …'` / `make console-test` — because the Makefile owns per-worktree provisioning (containers, dynamic ports, `.tmp/e2e.env`); hand-rolled infra setup or raw `pytest tests/backend-integration` bypasses that isolation and collides with parallel agents.
