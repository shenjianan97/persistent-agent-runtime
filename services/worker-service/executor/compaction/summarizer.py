...
# ---------------------------------------------------------------------------
# Task-1 interaction shim.
#
# Task 1 (summarizer prompt hardening + output cap) adds
# ``SUMMARIZER_MAX_OUTPUT_TOKENS = 1500`` to
# ``executor/compaction/defaults.py``. Until that task lands, we fall back to
# a local constant so this module can be imported and exercised in isolation.
#
# MERGE-TIME CLEANUP: the orchestrator MUST replace the try/except below with
# a plain ``from executor.compaction.defaults import SUMMARIZER_MAX_OUTPUT_TOKENS``
# import once Task 1 has been merged. The numeric fallback here is kept
# byte-identical to Task 1's value so the import-failure path and the
# import-success path produce the same payload-gate behaviour.
# ---------------------------------------------------------------------------
try:
    from executor.compaction.defaults import (  # type: ignore[attr-defined]
        SUMMARIZER_MAX_OUTPUT_TOKENS as _SUMMARIZER_MAX_OUTPUT_TOKENS,
    )
except ImportError:  # pragma: no cover — Task-1 interaction shim, removed at merge
    _SUMMARIZER_MAX_OUTPUT_TOKENS = 1500
