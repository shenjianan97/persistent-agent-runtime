...
#### A2. Impacted Components / Modules
  ...
  Description: Implement the exact Phase 1 PostgreSQL schema and key query support described in `docs/design-docs/phase-1/design.md`, including `tasks`, `checkpoints`, and `checkpoint_writes`, queue/reaper/dead-letter indexes, `updated_at` maintenance, and `LISTEN/NOTIFY` support for claimable task transitions.
...
When assigning tasks to implementation agents:
  ...
  Use `docs/design-docs/phase-1/design.md` as the canonical architecture/behavior contract and `agent_tasks/*.md` as the task-local implementation contract.
