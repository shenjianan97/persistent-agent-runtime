...
1. `docs/product-specs/index.md` 
2. `docs/design-docs/phase-1/design.md`

**CRITICAL POST-WORK:** After completing this task, you MUST update the status of this task to "Done" in the `docs/exec-plans/completed/phase-1/progress.md` file.

## Context
Phase 1 of the Persistent Agent Runtime relies on a Database-as-a-Queue model to eliminate dual-write hazards. We use PostgreSQL to serialize durable graph engine state (via checkpoints) alongside a `tasks` queue architecture, managed comprehensively by lease-ownership paradigms and structured tables. Refer to design.md (Phase 1 design doc) under Database Schema & Key Queries (Section 6.1) for complete design constructs.

## Task-Specific Shared Contract
- Treat `docs/design-docs/phase-1/design.md` as the canonical schema contract. Do not invent additional statuses, dead-letter reasons, or retry semantics.
