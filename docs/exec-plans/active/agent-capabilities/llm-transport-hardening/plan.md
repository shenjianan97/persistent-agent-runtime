...
**Tech Stack:** Python (worker, langchain-aws, botocore, asyncpg) · TypeScript / React 19 (Console) · PostgreSQL (conversation_log entries; no schema migration — `ConversationLogKind` is a Python enum, not a DB CHECK constraint, see `core/conversation_log_repository.py:_VALID_KINDS`).
...
| Conversation log kind | `services/worker-service/core/conversation_log_repository.py` | modification | Add two literals to `ConversationLogKind`: `llm_stream_progress` and `llm_stream_complete`. Update `_VALID_KINDS`. No DB migration — kind is enforced in Python only. |
...
- [ ] Worker logs show structured `llm_stream_progress` events within ≤ 5 s of `pre_call` for any LLM round, on both a fast model and a slow model.
