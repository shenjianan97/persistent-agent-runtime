...
| P3 | `GET /v1/tasks/{id}/plan` read-only projection | In progress | depends on P1 (item shape); Java-only; **added scope (P1 review, 2026-06-06):** add `plan_write` to `ValidationConstants.ALLOWED_TOOLS` + validation test — without it the tool is unreachable via agent config and P5 dead-ends |
