...
**Disposition status (2026-06-09, S11 acceptance):** **All three rows CLOSED** (v1 sufficient — metrics above). The §A12 definition-of-done gate is satisfied. One **non-ledger observability follow-up** surfaced during S11 — **now FIXED (2026-06-09)**, see below.

**Sub-agent-tree iteration grouping defect — FIXED (2026-06-09):** the `iteration` field on `marker.subagent.*` events was statically `0` for every round, because `executor/supervisor/graph.py::_fanout_node` read it from the once-injected `config['configurable']['iteration']` (set once by S8's `_inject_supervisor_configurable` at `execute_task` time, never advanced per round) instead of the live per-round state. As a `Send` target, `_fanout_node` only receives the `Send` payload, not the full `SupervisorState`, so it could not read `state["iteration"]` directly. S10's Console `buildSubagentTree` groups on `item.event.iteration ?? 0`, so all rounds' sub-agents collapsed into "Round 0" while `marker.supervisor.iteration` carried the real round. **Fix (minimal, one spot):** `_fanout_edge` (which DOES have the full live `state` with the current `iteration` minted by `supervisor_node`) now includes `iteration` in each `Send` payload, and `_fanout_node` reads `iteration = int(state.get("iteration", 0) or 0)` from the payload. Live state is authoritative — NOT the `"<iteration>.<index>"` subtask id, because a re-dispatched (failed-then-retried) subtask carries its ORIGINAL id forward for cross-round linkage, so parsing the id would give the original round, not the current one. Verified against `langgraph==1.0.5` (throwaway spike): `iteration` is a `_max_reducer` channel, every branch's payload carries the same current value, `_fanout_node` never writes `iteration` on output, so the channel/iteration-edge are unregressed. **S10 needed NO change** — `buildSubagentTree` already groups on `event.iteration`, and the `ActivityPane` unit fixture already encoded correct per-round iterations (round 1 → 1, round 2 → 2). Tests: `test_supervisor_fanout_events.py` updated to assert distinct live iteration per round (+ a re-dispatched subtask keeping its round-1 id while its round-2 marker carries iteration=2); `test_supervisor_citations.py` direct-`_fanout_node` fixture moved `iteration` from config into the Send payload to match the new contract.
