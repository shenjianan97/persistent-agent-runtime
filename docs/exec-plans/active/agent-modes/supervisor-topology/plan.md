...
| worker fan-out | `_insert_task_event` | `subagent_*` / `supervisor_iteration` + `{iteration, subtask}` details | event write in same txn as state mutation |
...
**Genuinely open (carried from design, each owned by a task):** Writer reduction algorithm (S7), checkpoint payload size cap (S6), finer in-flight metering (S8 notes, deferred).
...
**Spike #5 — durability against REAL Postgres, across a process boundary (2026-06-05).** Removes the MemorySaver caveat that hung over spikes #2–#4. Run against a throwaway `AsyncPostgresSaver`: worker A ran a 2-way `Send` fan-out, crashed one branch at inner step `w2`, then **closed its connection** (simulating process death). A **fresh worker B — new graph object, new Postgres connection, zero shared in-memory state** — resumed the same `thread_id`: the completed branch `t0` was untouched (`w1=w2=w3=1`), the crashed branch `t1` resumed **at `w2`** (`w1` not recomputed, `w2` re-ran, `w3` completed). **→ Per-inner-turn sub-agent durability holds on the real Postgres backend across a true process boundary, and message state serialized/round-tripped correctly. The Postgres-confirmation caveat in S3/S11 is satisfied at the LangGraph-checkpointer level** (the repo's custom `PostgresDurableCheckpointer` still gets one integration check in S11 with the real worker).
...
|---|---|---|---|---|---|
| D1 | **S6 — wide-fan-out checkpoint size cap/offload** | Log the serialized `subagent_results`/fan-out payload byte size past a threshold (`checkpoint.oversized` warning); **no cap**. | The warning fires in staging, or the **max checkpoint bytes** observed in the S11 load run exceeds a Postgres-write-latency budget. | **S11 acceptance** (report max bytes) → **pre-GA** of the `research` preset. | **OPEN** |
| D2 | **S7 — one-shot Writer reduction algorithm** | Hard cap on findings into the Writer + `log()` what's dropped (immutability invariant preserved). | **Cap-hit rate** + a **report-quality spot check** on real-ish research runs (does dropping findings degrade the report?). | **S11 acceptance** (report cap-hit rate + quality) → **pre-GA**. *Most likely to need action.* | **OPEN** |
| D3 | **S8 — finer in-flight cost metering** | Meter at the parent fan-out super-step boundary; per-sub-agent ceiling bounds overshoot to `max_fanout × ceiling`. | A cost-accuracy complaint, or observed per-task overshoot beyond `max_fanout × ceiling`. | **Post-GA** budget-accuracy monitoring (lowest priority). | **OPEN** |
