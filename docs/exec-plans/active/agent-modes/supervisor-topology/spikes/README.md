...
| `spike5_postgres_cross_process_resume.py` | Does per-turn resume hold on **real Postgres**, across a **process boundary** (fresh graph + fresh connection)? *Needs a throwaway Postgres:* `docker run -d --name par-spike-pg -e POSTGRES_PASSWORD=spike -e POSTGRES_DB=spike -p 55999:5432 postgres:16` (remove after). | Yes — removes the MemorySaver caveat |
