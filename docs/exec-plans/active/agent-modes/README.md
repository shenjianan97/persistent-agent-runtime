...
The Agent Modes design spawns several implementation tracks. Two flowed from it — each **independent, separately reviewed, and separately archived** — and **both are now complete and archived under [`completed/agent-modes/`](../../completed/agent-modes/)**; this index remains as their map. The Workflow resource and additional presets are explicitly Phase-3 and out of scope for both.

| Track | What it builds | Size / risk | Status |
|---|---|---|---|
| [Supervisor Topology](../../completed/agent-modes/supervisor-topology/plan.md) | Deep Research multi-agent fan-out (Scope → Supervisor → Subagents → Writer), the shared in-process fan-out helper, the `dispatch_subagent` tool, and presets | 11 tasks; larger, carries the cost/fan-out blockers (see its §A11) | **Done & merged (#119, 2026-06-20)** — archived to [`completed/agent-modes/supervisor-topology`](../../completed/agent-modes/supervisor-topology/). S1–S11 + post-S11 hardening (subagent_completed marker, source_allowlist enforcement, cancel_event propagation); §A12 ledger fully dispositioned. |
