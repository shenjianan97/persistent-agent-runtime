...
| Task 13 | User-Facing Conversation Log | Slice A done; B & C not started | Separate append-only `task_conversation_log` store so compaction Tiers 0/1/1.5 are invisible to customers; Tier 3 renders as an inline summary boundary (Claude-Code-style). New table + migration 0017, `GET /v1/tasks/{id}/conversation`, Console "Conversation" pane alongside CheckpointTimeline. **Slice A (DB + Worker) done:** migration `0017_task_conversation_log.sql`, `core/conversation_log_repository.py` (append-only, best-effort failure envelope), dual-write in `agent_node` (pre-LLM turns + agent_turn/tool_call + compaction_boundary/memory_flush), `hitl_pause`/`hitl_resume` hooks in `_handle_interrupt_internal` and the resume path. 24 new tests (12 repo + 12 integration) pass; full worker suite 1023/1023 green. Slices B (Java API read path) and C (Console Conversation pane) remain. |
