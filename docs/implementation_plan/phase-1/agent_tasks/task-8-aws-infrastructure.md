...
- Treat `docs/PROJECT.md` and `docs/design/PHASE1_DURABLE_EXECUTION.md` as the canonical infrastructure direction. This task should not reopen infrastructure choices already made.
...
- **File paths (if known):** `infrastructure/cdk/`, `services/api-service/`, `services/worker-service/`
...
Step 1: Bootstrap an AWS CDK project in TypeScript inside the `infrastructure/cdk/` directory, matching the project-level IaC decision in `docs/PROJECT.md`.
...
Step 8: Define a clean mechanism for schema initialization that is decoupled from application startup, consistent with the rollout guidance in `docs/implementation_plan/phase-1/plan.md`.
