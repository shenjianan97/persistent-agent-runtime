...
Runs a focused ReAct sub-agent **in-process** as a LangGraph **subgraph node reached via `Send`** (the post-agent routing edge intercepts the tool call — see *Shared fan-out machinery*) with its own context window and tool allowlist, and returns a structured summary to the parent as a `ToolMessage`. The `budget` arg is the **per-sub-agent ceiling** (token + turn cap) enforced in graph state by the shared fan-out helper — see *Shared fan-out machinery* for why this is load-bearing, not cosmetic. The sub-agent is *not* a separate task — it executes within the parent's run (same `thread_id`, same checkpoint, same task row); see *Execution model*. Its work is durable as part of the parent's graph state (its inner turns are checkpointed under a namespace — crash-resumable per turn), and its activity surfaces as sub-steps in the parent's event timeline, not as a separate Console row.
...
Why expose this as a *tool* and not a topology: ReAct agents sometimes benefit from delegating a focused subtask without polluting their own context (e.g., "investigate why test X is flaky"). Exposing it as a tool keeps the agent **Topology 1 (ReAct)** — the LLM decides *whether* and *when* to delegate (vs. the Supervisor's structural guarantee). The graph does gain the shared subagent node + a routing edge to reach it via `Send`, but the agent's shape and control flow remain ReAct. *Not a substitute for the Supervisor topology — see below.*
...
Both topologies fan out **in-process**, running sub-agents inside the parent's run rather than spawning separate durable child tasks. A sub-agent is a **compiled subgraph declared as a node** of the parent graph and reached by LangGraph's `Send` map-reduce primitive, sharing the parent's checkpointer (a namespaced sub-checkpoint per branch) — same `thread_id`, same task row. The parent's `ainvoke` drives the whole fan-out to completion before returning. This is the officially documented LangGraph approach to parallelism ([Send / graph API](https://docs.langchain.com/oss/python/langgraph/use-graph-api)).

**Wiring requirement (load-bearing — verified against `langgraph==1.0.5`, spike 2026-06-05).** The sub-agent must be a **subgraph node reached by `Send`**, *not* a subgraph `ainvoke`d imperatively inside a tool/node function. The distinction is the whole durability story: a subgraph-node shares the checkpointer and its **internal** super-steps are checkpointed under a namespace, so a crash mid-sub-agent resumes **per inner turn** (completed turns restored, only the interrupted turn re-runs). An imperatively-`ainvoke`d subgraph runs *inside one parent super-step* and is opaque to the checkpoint boundary until it returns — a crash re-runs the whole sub-agent from scratch. **Both drivers therefore route through `Send` to the same shared subagent node** (see *Shared fan-out machinery*); the earlier "gathered `ainvoke`" option is rejected because it forfeits per-turn durability.
...
- **Crash durability as graph state — per inner turn, not per whole sub-agent.** LangGraph checkpoints at super-step boundaries and records per-node `pending_writes` (persisted in `pregel/_loop.py`, short-circuited on resume in `pregel/_runner.py`). Because each sub-agent is a checkpointed subgraph node (see *Wiring requirement*), its **inner** turns are super-steps too: a worker crash mid-fan-out resumes the parent's run and (a) does **not** recompute completed sibling sub-agents, and (b) resumes a sub-agent that was mid-run **at the turn it died on** — its earlier turns are restored, not redone, and their tokens are not re-spent. *Spike-verified (2026-06-05): a 3-way `Send` fan-out where one branch crashed at its 3rd inner step resumed that branch at step 3 while leaving the other two untouched.* (LangGraph's *default* durability is `"async"`; this runtime invokes with `durability="sync"` — `executor/graph.py:3127` — persisting each super-step synchronously before the next, which is *stronger*, not weaker.)
- **Sub-agent transcripts are persisted (and readable).** Because the sub-agent subgraph's state — including its working message list — is checkpointed under its namespace, the full sub-agent transcript survives in the checkpoint store. The Console can surface it on demand (drill-in) by reading the namespaced sub-checkpoint — **no separate transcript store or table is required** (see *Observability*).
...
The in-process fan-out helper — run a ReAct sub-agent subgraph with isolated context + tool allowlist, return a structured summary — is the **same** for both topologies, and **both drivers route through `Send` to the same shared subagent node** (so both inherit the per-turn durability of *Execution model*). **Built once, used by both.** The *driver* differs only in **who triggers the `Send`** and **how the result is injected back**:

| | ReAct + `dispatch_subagent` (Topology 1) | Supervisor (Topology 2) |
|---|---|---|
| Who triggers the `Send` | The **routing edge after the agent node**, on seeing a `dispatch_subagent` tool call the LLM emitted | The **graph**, reading the Supervisor's structured subtask list |
| Count | LLM-emergent (how many `dispatch_subagent` calls the model makes in a turn) | Supervisor-decided per iteration, within `max_fanout_per_iteration` |
| Failure mode | LLM might not call the tool → no fan-out | Structurally guaranteed to fan out |
| Fan-out mechanism | `Send` to the subagent node (a routing edge intercepts the tool call — it is **not** executed inside the ToolNode) | `Send` to the subagent node, from the Supervisor's routing |
| Result injection | A `ToolMessage` (carrying the original `tool_call_id`) appended to the parent's message history | Typed state field (`state["subagent_results"]`) the Supervisor node reads |
...
**Why `dispatch_subagent` routes through `Send` rather than the ToolNode.** Running the sub-agent inside the ToolNode would bury its whole multi-turn run in a single parent super-step — forfeiting the per-turn crash-resume durability (a mid-run crash would re-run the entire sub-agent). Instead, the routing edge after the agent node intercepts a `dispatch_subagent` tool call and `Send`s it to the shared subagent node, exactly as the Supervisor does; the subagent node threads its summary back as a `ToolMessage` keyed to the `tool_call_id` while keeping its internal working messages on a **separate channel** (preserving context isolation — the parent sees only the summary). This is the same controllability argument that made Supervisor a topology rather than a tool, applied one level deeper, and it makes durability **uniform across both drivers**. Share machinery, differ on driver.
...
- **Crash recovery** is checkpoint-based and **fine-grained**: a worker dying mid-fan-out re-claims the parent task and resumes from the last super-step checkpoint. Completed sub-agents are not re-run, and a sub-agent caught mid-run resumes at the inner turn it died on (its earlier turns were checkpointed under its namespace — see *Execution model*). No separate child tasks to reap.
...
**Two layers of sub-agent detail, both already persisted — no new store.** (1) The `task_events` markers above give the lightweight skeleton (round → which sub-agent started/finished/failed). (2) Each sub-agent's *full transcript* (its turn-by-turn working messages) lives in its **namespaced sub-checkpoint** (see *Execution model* — it is checkpointed for durability regardless). The Console drill-in ("click a sub-agent to see its logs") reads that namespaced checkpoint on demand; it does **not** require duplicating the transcript into `task_events` or a new table. The only cost is a read path that descends into the sub-checkpoint namespace (the default activity projection reads the parent's top-level `messages` channel only). Trade-off: persisting every sub-agent's full inner state enlarges the per-super-step checkpoint write on a wide fan-out — a size-management concern (cap/offload if it grows), tracked in *Open decisions*, not a blocker.
...
- **2026-06-05** (spike-verified durability refinement): **Sub-agents are checkpointed subgraph nodes reached by `Send` — and both drivers use that path.** Verified against `langgraph==1.0.5` (throwaway spikes): a `Send` fan-out to a subgraph-node persists each branch's **inner** super-steps under a namespace, so a crash mid-sub-agent resumes **per inner turn** (a 3-way fan-out crashed at one branch's 3rd step resumed that branch at step 3, leaving the others untouched), and the sub-agent's full transcript survives in its sub-checkpoint. Two consequences: (1) the earlier "or gathered subgraph `ainvoke`" wiring is **rejected** — an imperatively-invoked subgraph runs inside one parent super-step and forfeits per-turn durability; (2) `dispatch_subagent` (Topology 1) is **routed through `Send`** by the post-agent routing edge (intercepting the tool call), *not* executed inside the ToolNode — so durability is uniform across both drivers, and its result threads back as a `ToolMessage` (matching `tool_call_id`) while internal working messages stay on a separate channel. *(This dispatch path was itself end-to-end spike-verified: tool-call→`Send`→subagent-node→`ToolMessage` threading, context isolation via a non-parent-schema internal channel, mixed-turn answering [dispatch via `Send` + a normal tool via the ToolNode, every `tool_call_id` answered once], and a mid-sub-agent crash resuming per inner step — all pass on `langgraph==1.0.5`.)* This also resolves sub-agent observability: the Console drill-in reads the namespaced sub-checkpoint — **no new transcript table**. The crash-recovery wording above is refined from "re-executes only the unfinished sub-agents" to per-inner-turn resume; the wide-fan-out checkpoint-size item stays open (now the cost of persisting full inner state, not just results).
