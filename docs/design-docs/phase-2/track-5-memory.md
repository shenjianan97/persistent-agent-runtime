...
- **Track 2 (Runtime State Model):** Terminal states (`completed`, `dead_letter`) are already defined; cancellation is modeled as `status = 'dead_letter'` with `dead_letter_reason = 'cancelled_by_user'` — not a distinct status. Memory write reads final graph state and task terminal state; no new status values are introduced.
