...
For later-phase concepts beyond Phase 1, see [design.md](../phase-2/design.md) for the consolidated Phase 2 design and [design-notes.md](../phase-3-plus/design-notes.md) for Phase 3+ reference material.
...
**Phase 1 scaling model:** The Worker Service is single-threaded (`asyncio`) and I/O-bound — it spends most of its time waiting on LLM API calls and database queries, not consuming CPU. CPU-based autoscaling would not reflect actual worker load. Queue-depth-based autoscaling requires a custom CloudWatch metric publisher, which adds complexity beyond Phase 1 scope. Therefore, **Phase 1 uses a fixed Worker Service instance count** (configurable at deploy time, default 1). Autoscaling based on queue depth is deferred to Phase 2. The API Service uses CPU-based autoscaling since it is request-driven.
...
Phase 1 has five deployable components. Understanding their boundaries is essential before diving into execution details.
...
The **API Service** and **Worker Service** are independent processes that share nothing except the database. Multiple instances of each can run concurrently. Neither holds in-memory state that would be lost on crash — all durable state lives in PostgreSQL. The **Console** is a static SPA packaged into a container and served behind the same internal ALB as the API, allowing same-origin browser requests without browser-held credentials when reached through the operator access path. The **Model Discovery** Lambda runs on a schedule and is also invoked once during environment bootstrap so a fresh deployment has a populated `models` table before first use.
...
- **Secret isolation:** In Phase 1, sensitive values live in AWS Secrets Manager and are injected only into the compute components that actually need them. This includes database credentials (auto-generated by Aurora), tool API keys such as `TAVILY_API_KEY` for the Worker Service, and LLM provider API keys such as `ANTHROPIC_API_KEY` / `OPENAI_API_KEY` for the Model Discovery Lambda. The browser-based Console receives no secrets. Services consume these values as standard environment variables at runtime — no application-level Secrets Manager SDK integration is required. Secrets must never be inserted into LangGraph state, checkpoint payloads, metadata, or logs. The checkpointer does not mutate persisted state; log redaction happens in the logging layer, and code review/tests enforce that secrets never enter checkpointed state.
...
#### Secret Handling
- In Phase 1, database credentials and external API keys are stored in AWS Secrets Manager and injected into the specific runtime components that need them as environment variables. Never place secrets in checkpoint payloads or agent config.
- Tool handlers receive secrets from process environment at execution time. For example, the Worker Service receives `TAVILY_API_KEY`, while LLM provider keys are supplied to Model Discovery and persisted into `provider_keys` for runtime lookup.
...
- Phase 1 does not implement application-layer authentication/authorization. Access is expected to be restricted at the deployment layer, for example by exposing the API and Console only through an internal ALB reached via an operator access path such as an SSM-managed jump host.
...
1. Add a routine to `services/model-discovery/main.py` (or a similar future registry tool) to list AWS Bedrock models via `boto3`.
