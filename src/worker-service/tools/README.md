...
- `src/worker-service/tools/.env`
- `src/worker-service/.env`
...
Example `src/worker-service/tools/.env`:
...
```bash
cd .
src/worker-service/.venv/bin/pytest src/worker-service/tests -q
```
...
```bash
cd .
src/worker-service/.venv/bin/pytest \
  src/worker-service/tests/test_env_loading.py \
  src/worker-service/tests/test_mcp_server.py \
  src/worker-service/tests/test_mcp_http_integration.py \
  src/worker-service/tests/test_mcp_stdio_integration.py -q
```
...
- `app.py` is intentionally the portable assembly layer so this package can move into a future standalone `src/mcp-server/` module with minimal changes.
